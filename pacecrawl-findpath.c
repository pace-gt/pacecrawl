...
int main(int argc, char *argv[]) {
    ...
    key.mv_data = (void *)path;
    key.mv_size = strlen(path) + 1;

    if ((err = mdb_cursor_get(cursors.pathsCursor, &key, &val, MDB_SET)) !=
        MDB_SUCCESS) {
        fprintf(stderr, "ERROR: failed to find path \"%s\": %s\n", path,
                mdb_strerror(err));

        return EXIT_FAILURE;
    }

    uint64_t inode = *((uint64_t *)val.mv_data);
    ...
    fprintf(
        ...
        "atime=%zu, btime=%zu, size=%zu, blocks=%zu, inode=%zu, nhlink=%zu}\n",
        it->mode, it->uid, it->gid, it->modtime.tv_sec, it->chgtime.tv_nsec,
        it->atime.tv_sec, it->btime.tv_sec, it->size, it->blocks, it->inode, it->nhlink);
